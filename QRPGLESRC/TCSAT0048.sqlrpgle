**Free
  ...
  Dcl-DS $DsRcdO  Extname('FINPLTDAT/PLTCTACUP') Prefix(O_);
  End-DS;
  Dcl-DS *N  PSDS;
    @pgma          Char(10)   Pos(1);
    @job           Char(10)   Pos(244);
  ...
  $Parm1a = $Parm1;
  if  ($Evento = '3') 
      And vlrcup > 0 and FecFin = 0;
      $RcdNw = *blanks;
      IF $DesRcdNw>0 and $LenRcdNw>0;
        $RcdNw = %subst($AllParm1 :$DesRcdNw+1 :$LenRcdNw);
      endif
      IF $DesRcdOr>0 and $LenRcdOr>0;
        $RcdOr = %subst($AllParm1 :$DesRcdOr+1 :$LenRcdOr);
      endif;
      $DsRcd = $RcdNw;
      $DsRcdO = $RcdOr;

      If VlrCup > 0 and O_VlrCup = 0 and FecFin = 0;
        sw_existe = *Off;
        sw_existe = *Off;
        Exec Sql  
